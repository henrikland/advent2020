...
for y, line in enumerate(lines):
    for x, char in enumerate(line):
        grid[MIDDLE_INDEX + x][MIDDLE_INDEX +
                               y][MIDDLE_INDEX][MIDDLE_INDEX] = char == "#"


def check_cube(grid, cube_x, cube_y, cube_z, cube_w):
    cube_active = grid[cube_x][cube_y][cube_z][cube_w]
        ...
        for y in neighbors:
            for z in neighbors:
                for w in neighbors:
                    ...
                    w_outside = neighbor_w < 0 or neighbor_w > LENGTH_WITH_SPACE - 1

                    if x_outside or y_outside or z_outside or w_outside:
                        continue

                    if grid[neighbor_x][neighbor_y][neighbor_z][neighbor_w]:
                        active_neighbors += 1
    ...
    for x in range(LENGTH_WITH_SPACE):
        for y in range(LENGTH_WITH_SPACE):
            for z in range(LENGTH_WITH_SPACE):
                for w in range(LENGTH_WITH_SPACE):
                    fresh_grid[x][y][z][w] = check_cube(grid, x, y, z, w)
